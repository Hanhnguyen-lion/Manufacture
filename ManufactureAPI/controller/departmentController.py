...
async def create_department(department:Department, request: Request):
    ...
    if new_tem is not None:
        return departmentEntity(request.app.database.M_Company.find_one({"_id": ObjectId(new_tem.inserted_id)}))
...
async def find_all_departments(request: Request):
    return departmentsEntity(request.app.database.M_Department.find(limit=100))
